...
26 年實證 (1:1 口, 同點值 $50; z>1.5 進場、|z|<0.3 出場; 實際採用 WIN=120 日窗
『短線版』, 較活躍; 數字隨資料每次動態重算, 以下為近期實跑值):
  黃豆-小麥價差  勝率72% PF2.31 (93筆)
  小麥-玉米價差  勝率81% PF3.18 (94筆)
  黃豆-玉米價差  勝率67% PF1.15 (79筆, 最弱)
(長線 252 日窗風險報酬更佳但交易較少: 80%/PF4.6 等; 為求短線活躍採 120 日。)

抗過擬合: WIN=120 樣本內外對切 (前/後半) 兩段皆正且 PF 接近 (黃豆-小麥 +2.25/+2.24);
z 門檻 1.0~2.5 為平滑平台, 非知識邊緣。故參數採常規固定值, 不做網格挑選。

注意: current_z (即時) 與 backtest 均用『不含當日』視窗算 z, 兩端一致 (見 current_z)。
DD 為固定 25 萬本金/1 口基底的%, 僅供相對比較, 非絕對風險。
...
def current_z(legA, legB, win=WIN):
    ...
    if len(sp) < win + 1:
        return None
    m = sp[-win - 1:-1].mean()      # 不含當日 (與 backtest sp[i-win:i] 一致)
    s = sp[-win - 1:-1].std()
    if s <= 0:
        return None
