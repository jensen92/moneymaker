...
無一同時通過兩關 —— 故生產參數維持不變, 未加任何濾網:
  ...
  L2 ATR天花板: 初看是唯一日線降DD者, 但過擬合複驗推翻 —— 那個增益只在【次優base】
     (小時參數硬套日線) 成立; 換到日線【實際採用配置 bo40/atr4】後幾乎只砍利、DD不降
     (屬對弱基準的相對改善=過擬合)。小時線上更直接傷績效。否決, 且不併入 (已移除機制)。
