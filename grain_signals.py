...
def main():
    ...
    for key in ("ZS", "ZC"):
        ...
        if s["in_entry"]:
            ...
            print(f"  👉 操作: 今日市價買進, 同時掛停損賣單 {s['stop']:,.1f}(GTC)。"
                  f"之後不動, 等到期或停損")
        elif s["holding"]:
            print(f"  🟡 持有中｜進場 {s['entry']:,.1f}｜停損 {s['stop']:,.1f}｜"
                  f"{s['exit_m']}月到期（剩約{s['days_left']}交易日）")
            print(f"  👉 操作: 續抱不動。確認停損單仍掛 {s['stop']:,.1f}(不移動); "
    ...
    print("\n💡 概念: 收割壓力打出年度低點後買進, 吃庫存消化+隔年播種/天候溢價的季節上漲段。")
    print("   寬停損(3×ATR)只防黑天鵝, 期間震盪不出場; 出場只有兩種: 到期月平倉 或 觸停損。")
    print("   (一年一次; 已驗證收緊停損/移動停損反而砍贏家, 故規則就是「進場後放著」)")
